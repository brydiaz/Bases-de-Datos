<!DOCTYPE html>
<html lang="esp">
    <head>
     
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
        <script src="http://code.jquery.com/jquery-1.7.1.min.js"></script>

    </head>
        <body>
            <div id='Contenido'>
                <header>
                   <hgroup>
                       <h1>ATD32 system</h1>
                   </hgroup> 
                </header>
                <!-- Sidebar -->
                <div class="w3-sidebar w3-light-grey w3-bar-block" style="width:25%">
                  <h3 class="w3-bar-item">Options</h3>
              
                  <a href="showDepartments.php" class="w3-bar-item w3-button">Show Departments</a>
                  <a href="showWorkers.php" class="w3-bar-item w3-button">Show Workers</a>
                  <a href="insertWorkers.php" class="w3-bar-item w3-button">Insert Workers</a>
                  <a href="insertDeparments.php" class="w3-bar-item w3-button">Insert Deparments</a>
                  <a href="deleteDeparments.php" class="w3-bar-item w3-button">Delete Deparments</a>

                  <a href="editWorker.php" class="w3-bar-item w3-button">Edit Worker</a>
                  <a href="editDeparment.php" class="w3-bar-item w3-button">Edit Deparment</a>

                  <a href="deleteDeparments.php" class="w3-bar-item w3-button">Delete Workers</a>
                  <a href="deleteWorker.php" class="w3-bar-item w3-button">Delete Worker</a>
                  <a href="deleteDeparments.php" class="w3-bar-item w3-button">Delete Deparments</a>

                  <a href="developers.php" class="w3-bar-item w3-button">Developers</a>

                </div>

                <!-- Page Content -->
                <div style="margin-left:25%">

                <div class="w3-container w3-teal">
                  <h1>Future of Tecnology</h1>
                </div>
                Como motivo del entendimiento de las bases de datos NoSQL, el equipo
                de ADT32 se propuso como mision y además Investigacion calificada crear
                un CRUD( "Crear, Leer, Actualizar y Borrar" (del original en inglés: Create, Read, Update and Delete))
                totalmente funcional con MongoDB usamos como ejemplo una base de datos simple de una comañia con 
                departamentos y empleados. Esperemos sea de su agrado. <br></br>
                -Futuros Ingenieros del Mundo, ADT32
                <br></br>
                <img src="images/logoADT.png" width="300" height="241">

            </div>
        
        </body>

</html>





